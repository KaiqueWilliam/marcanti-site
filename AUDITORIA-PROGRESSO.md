...
- [x] **Schema** → Etapa 10: `Organization`+`LocalBusiness` na home (endereço, telefone, horário, redes, fundação), `Product` nas 11 páginas de produto, `BreadcrumbList` nas 18 páginas com trilha. Validado: 90 blocos JSON-LD, todos parseando, `Product.url == canonical` e `Product.description == meta description`. **Falta o CNPJ** (`taxID`) e **`FAQPage`**, que só entra quando existir bloco de perguntas na página (audit 4.5) (audit 8)
...
- [ ] **BLOQUEADO (seção 9 #4)** — Quem Somos: "quatro máquinas de injeção". Mantido de propósito na Etapa 11 (a auditoria sugere remover, mas isso é decisão do cliente). Manter como prova de porte ou trocar por formulação neutra?
- [ ] **BLOQUEADO (seção 9 #1)** — Home + Sustentabilidade: "matéria-prima 100% reciclável" vs "reciclada" do material interno. A copy da Etapa 11 está escrita na versão "reciclável"; se for "reciclada", é troca de palavra em 2 arquivos.
...
2. [ ] Missão e visão — site atual vs material institucional
