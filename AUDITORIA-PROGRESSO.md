...
- [x] **Imagens pesadas → WebP/AVIF** → já resolvido neste repo antes da auditoria começar: 85 das 86 imagens têm AVIF+WebP gerados e servidos via `<picture>`; a exceção (`logo-marcanti.png`) é deliberada no script, porque a marca amassa com compressão com perda. Medido: 7,9 MB de originais → 1,9 MB entregues, 76% de economia (audit 8)
